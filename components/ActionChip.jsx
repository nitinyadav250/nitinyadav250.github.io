...
import {RATE_COURSE, AVATAR_URL, ATTENDANCE} from "../config/constants";
...
export const WriteReviewChip = ({className, subject}) => {
    ...
    useEffect(() => {
        setRollNo(localStorage.getItem('roll_no'));
    ...
    const handleSubmit = async (e) => {
        ...
        console.log(review, subject, rollNo);
        await axios
            .post(ATTENDANCE, {
                code: review,
                subject,
    ...
    useEffect(() => {
        switch (rating) {
            case 1:
                setComment('Terrible Class 😡');
                break;
            case 2:
                setComment('Bad Class 😒');
                break;
            case 3:
                setComment('Average Class 😐');
                break;
            case 4:
                setComment('Good Class 😄');
                break;
            case 5:
                setComment('Excellent Class 🤩');
                break;
            default:
                setComment('How was the Class?');
        }
    }, [rating]);
    ...
    return (
        <>
            <div className={'fixed top-14 left-0 right-0 z-50'}>
                {alert && (
                    ...
                    >
                        ...
                        Your Attendance have been submitted!
                    </Alert>
                )}
            </div>
            <div className={"border-1 flex rounded-lg px-2 items-center justify-center cursor-pointer  text-white bg-gray-800 hover:bg-black duration-300 " + className} onClick={handleOpen}>
                <span className="ml-1 text-lg p-5">Attendance</span>
            </div>
            <Modal
                open={open}
                onClose={handleClose}
            ...
            >
                <Box sx={style}>
                    <Typography id="modal-modal-title" variant="h6" component="h2" className={''}>
                        Please Enter the Attendance Code <p className='text-sm'></p>
                    </Typography>
